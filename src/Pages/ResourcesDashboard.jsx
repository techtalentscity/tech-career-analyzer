// src/Pages/ResourcesDashboard.jsx
...
const ResourcesDashboard = () => {
  ...
  useEffect(() => {
    ...
    const loadAnalysisAndResources = async () => {
      try {
        ...
        const analysis = await storageService.getCareerAnalysis(userId);
        if (!analysis) {
        ...
        if (activeTab === 'learning') {
          await loadLearningResources(userId, careerPathData);
        } else {
          await loadInterviewQuestions(userId, careerPathData);
        }
      } catch (error) {
  ...
  const loadLearningResources = async (userId, careerPathData) => {
    try {
      // Check if we already have the data in storage
      if (storageService.hasLearningResources(userId)) {
        setResources(await storageService.getLearningResources(userId));
      } else {
        // Get new data from Gemini
        const newResources = await geminiApiService.getLearningResources(careerPathData);
        await storageService.saveLearningResources(userId, newResources);
  ...
  const loadInterviewQuestions = async (userId, careerPathData) => {
    try {
      // Check if we already have the data in storage
      if (storageService.hasInterviewQuestions(userId)) {
        setInterviewQA(await storageService.getInterviewQuestions(userId));
      } else {
        // Get new data from Gemini
        const newInterviewQA = await geminiApiService.getInterviewQuestions(careerPathData);
        await storageService.saveInterviewQuestions(userId, newInterviewQA);
